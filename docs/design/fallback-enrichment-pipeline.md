...
**Confirmed search escalation** — if all prior stages yield `no_match` or `incomplete`, the runtime performs an open-web datasheet search (Brave Search API) and fetches the first resolvable PDF result. Extraction runs immediately. The resulting field candidates are stored as `authority_tier=web_search` and surfaced as pending field review rather than auto-applied. The outcome is `needs_confirmation`. The user confirms or rejects through the existing pending-review flow (`/inventory/{id}/accept` or `/inventory/{id}/dismiss`). This stage requires a `[search]` section in `config.toml`; no API key is needed (uses DuckDuckGo HTML search). If the section is absent, the stage is skipped.
