...
This unit is design-ready at medium readiness. The fallback chain, authority rules, provenance requirement, outcome model, and target field set are concrete. The remaining gaps are implementation-relevant details about conflict criteria and persistence shape, not uncertainty about the core design direction.
