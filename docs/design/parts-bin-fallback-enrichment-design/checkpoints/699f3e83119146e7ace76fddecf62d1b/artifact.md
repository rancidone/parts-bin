...
- Which exact field set is in scope for fallback enrichment beyond manufacturer, description, and package?
- What identifying fields count as a blocking conflict between LCSC and DigiKey?
- How much provenance should be persisted in the database versus kept only in runtime/log artifacts?

## Readiness

This unit is design-ready at medium readiness. The fallback chain, authority rules, provenance requirement, and outcome model are concrete. The remaining gaps are implementation-relevant details about exact field scope and persistence shape, not uncertainty about the core design direction.
