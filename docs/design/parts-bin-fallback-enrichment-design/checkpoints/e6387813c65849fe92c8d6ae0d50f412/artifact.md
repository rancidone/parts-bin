...
This unit is design-ready at high readiness. The fallback chain, authority rules, conflict criteria, outcome model, writable field set, and description-merge policy are now concrete. The remaining open question affects implementation depth, not the core behavior of the enrichment pipeline.
